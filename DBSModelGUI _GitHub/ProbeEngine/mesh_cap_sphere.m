function [Pcap, tcap, ncap] = mesh_cap_sphere(startend, Pcenter, P, t, M, a, b)
    ...
    %   Deform the planar mesh to form the cap
    dist            = vecnorm(Pcap');
    lift            = real(sqrt(radius^2 - dist.^2));
    Pcap            = Pcap + repmat(lift', 1, 3).*repmat(UnitPathVector, CapNondes, 1);
